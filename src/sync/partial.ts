...
import { loadIndex, writeIndex, type SyncIndex } from "./index.ts";
...
export async function partialSync(options: PartialSyncOptions): Promise<void> {
  ...
  for (const rawPageId of options.pageIds) {
    ...
    try {
      ...
      const existingState = index.pages[pageId] ?? index.pages[dashedId];
      ...
      const indexKey = existingState ? (index.pages[pageId] ? pageId : dashedId) : pageId;
